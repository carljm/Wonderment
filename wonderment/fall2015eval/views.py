...
def survey(request, parent_id, id_hash):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response.save()
            return redirect(
                'fall2015eval_done', parent_id=parent_id, id_hash=id_hash)
    else:
    ...
    return render(request, 'fall2015eval/form.html', context)
...
def done(request, parent_id, id_hash):
    ...
    return render(request, 'fall2015eval/done.html', context)
...
def results(request):
    ...
    return render(
        request, 'fall2015eval/results.html', {'summary': summary})
...
def teacher_survey(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('fall2015eval_teacher_done')
    else:
    ...
    return render(request, 'fall2015eval/teacher_form.html', context)


def teacher_done(request):
    return render(request, 'fall2015eval/teacher_done.html')
